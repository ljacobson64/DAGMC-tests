...
def write_html_head(writer):
    ...
    write_line(writer, 2, '<title>FluDAG testing diff summary</title>')
    write_line(writer, 1, '</head>')
    write_line(writer, 1, '<body>')
    write_line(writer, 2, '<h1>FluDAG testing diff summary</h1>')
...
def write_text_head(writer):
    writer.write('FluDAG testing diff summary\n\n')
    writer.write('%s\n\n' % datetime_1)
